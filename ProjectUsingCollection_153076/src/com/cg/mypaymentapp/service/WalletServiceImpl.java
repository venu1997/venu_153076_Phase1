...
public class WalletServiceImpl implements WalletService{
	...
	public Customer depositAmount(String mobileNo, BigDecimal amount) throws InvalidInputException{
		...
		if(isValidMobile(mobileNo) && isValidamount(amount)){
			...
			repo.save(customer);
			repo.saveTransaction(mobileNo, "you have deposited amount of RS."+amount+" on "+dtf.format(now));
		}
		return customer;
	}
	public Customer withdrawAmount(String mobileNo, BigDecimal amount) throws InsufficientBalanceException{
		...
		if(isValidMobile(mobileNo) && isValidamount(amount)){
			...
			customer.setWallet(new Wallet(balance));
			repo.save(customer);
			repo.saveTransaction(mobileNo, "you have withdrawn an amount of RS."+amount+" on "+dtf.format(now));
		}
		return customer;
	}
